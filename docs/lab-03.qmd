...
```{r}
state_data <- population_covid %>%
  group_by(state, date) %>%
  summarize(daily_new_cases = sum(new_cases, na.rm = TRUE))

state_data <- state_data %>%
  mutate(rolling_mean = rollmean(daily_new_cases, k = 7, fill = NA, align = "center"))

ggplot(state_data, aes(x = date)) +
  geom_bar(aes(y = daily_new_cases), stat = "identity", fill = "lightblue") +  # Bar plot for daily new cases
  geom_line(aes(y = rolling_mean), color = "red", size = 1) +  # Line for 7-day rolling mean
  facet_wrap(~ state, scales = "free_y") +  # Facet by state with free y-axis scales
  labs(title = "Daily New COVID Cases and 7-Day Rolling Mean", x = "Date", y = "New Cases") +
  theme_minimal() +
